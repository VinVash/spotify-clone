...
const useAudio = (nftAlbum) => {
  ...
  // gives us the current audio playing at any given time.
  const audioRef = useRef(new Audio(resolveLink(JSON.parse(audio[trackIndex].metadata).animation_url)));

  const intervalRef = useRef(); // keep track of current song being played and inrementing the duration that has passed the song.
  const isReady = useRef(false); // moving on to the next song if we are ready to move to the next song on the audio player.
